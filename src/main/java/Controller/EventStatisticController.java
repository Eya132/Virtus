...
import javafx.scene.control.Label;
    ...
    public class EventStatisticController {
        ...
        // Méthode pour gérer la participation d'un utilisateur
        public void handleParticipation(int userId) {
            if (participationService.hasUserParticipated(event.getIdevent(), userId)) {
                // Si l'utilisateur a déjà participé, afficher un message ou alerter
                System.out.println("L'utilisateur a déjà participé à cet événement.");
            } else {
                // Ajouter la participation de l'utilisateur
                participationService.addParticipation(event.getIdevent(), userId);
                System.out.println("Participation enregistrée.");
            }
        }
    }
